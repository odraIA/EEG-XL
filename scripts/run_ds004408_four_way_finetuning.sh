...
# Prepare ds004408 as word-aligned EEG and compare four initializations:
...
#
# ds004408 is always represented as physical EEG sensor type 2. The per-run
# embedding id only controls which row of sensor_type_layer is looked up.
...
echo $$ > "$PID_FILE"
printf 'order\tlabel\tembedding_id\tcontainer\tstatus\texit_code\tstarted_at\tfinished_at\n' > "$STATUS_FILE"
...
prepare_word_aligned() {
  ...
  append_status 0 prepare_word_aligned 2 docker-compose-run-rm COMPLETED 0 "$started" "$finished"
}

run_experiment() {
  local order="$1" label="$2" train_from_scratch="$3" init_checkpoint="$4"
  local embedding_id="$5" compare="$6"
  ...
  if [[ "$compare" == "1" ]]; then
    extra_env+=(
      -e "MEGXL_COMPARISON_RUNS=random_init=$LOG_ROOT/random_init;eeg_from_scratch=$LOG_ROOT/eeg_from_scratch;megxl_eeg2=$LOG_ROOT/megxl_eeg2;megxl_eeg1=$LOG_ROOT/megxl_eeg1"
      -e "MEGXL_COMPARISON_OUTPUT=$RESULTS_ROOT"
    )
  fi

  started="$(date --iso-8601=seconds)"
  echo "$container" > "$CURRENT_CONTAINER_FILE"
  ...
  finished="$(date --iso-8601=seconds)"
  if [[ "$exit_code" != "0" ]]; then
    append_status "$order" "$label" "$embedding_id" "$container" FAILED "$exit_code" "$started" "$finished"
  ...
  append_status "$order" "$label" "$embedding_id" "$container" COMPLETED "$exit_code" "$started" "$finished"
  docker rm "$container" >/dev/null
  CURRENT_CONTAINER=""
}

prepare_word_aligned
run_experiment 1 random_init true "$MEGXL_ARCH_CHECKPOINT" 2 0
run_experiment 2 eeg_from_scratch false "$FROM_SCRATCH_EEG_CHECKPOINT" 2 0
run_experiment 3 megxl_eeg2 false "$MEGXL_EEG2_CHECKPOINT" 2 0
run_experiment 4 megxl_eeg1 false "$MEGXL_EEG1_CHECKPOINT" 1 1
...
cp "$RESULTS_ROOT/weissbart_three_way_test_metrics.csv" "$RESULTS_ROOT/ds004408_four_way_test_metrics.csv"
